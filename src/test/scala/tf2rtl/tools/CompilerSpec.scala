...
import tf2rtl.tools.golden.ExecutiveTraceContext
import tf2rtl.tools.compiler.{MemoryDimensions, MemoryTag}
...
class CompilerSpec extends FlatSpec {
  ...
  it should "Compile TF Conv2D (VALID padding, 2x2 strides) 3x3x4 image with 2x2x4x4 kernel" in {
    ...
    Compiler.compile(
      name,
      "tf_models/conv2d_4x4_valid_stride_2.pb",
      List("Identity_1"),
      options
    )
  ...
  it should "Compile TF Conv2D (SAME padding, 2x2 strides) 3x3x4 image with 2x2x4x4 kernel" in {
    ...
    Compiler.compile(
      name,
      "tf_models/conv2d_4x4_same_stride_2.pb",
      List("Identity_1"),
      options
    )
  ...
  it should "Compile TF Conv2D (SAME padding) 3x3x4 image with 2x2x4x4 kernel, Relu, MaxPool (VALID padding)" in {
    ...
    Compiler.compile(
      name,
      "tf_models/conv2d_4x4_same_relu_2x2_maxpool_valid_stride_2.pb",
      List("Identity_1"),
      options
    )
  ...
  it should "Compile TF Conv2D (SAME padding) 3x3x4 image with 2x2x4x4 kernel, Relu, MaxPool (VALID padding, 1x1 stride)" in {
    ...
    Compiler.compile(
      name,
      "tf_models/conv2d_4x4_same_relu_2x2_maxpool_valid_stride_1.pb",
      List("Identity_1"),
      options
    )
  ...
  it should "print the compiled program for large" taggedAs (Slow) in {
    ...
    Compiler.compile(
      name,
      "tf_models/conv2d_3_224_64_128.pb",
      List("Identity"),
      options
    )
  }
  ...
  it should "Compile TF float ResNet50V2 (ImageNet)" taggedAs (Slow) in {
    ...
    Compiler.compile(
      name,
      "tf_models/resnet50v2_imagenet.pb",
      List("Identity"),
      options,
      traceContext
    )
  ...
  it should "Compile ONNX float ResNet20V2 (CIFAR)" taggedAs (Slow) in {
    ...
    Compiler.compile(
      name,
      "tf_models/resnet20v2_cifar.onnx",
      List("Identity:0"),
      options,
      traceContext
    )
  ...
  it should "Compile ONNX float ResNet50V2 (ImageNet)" taggedAs (Slow) in {
    ...
    Compiler.compile(
      name,
      "tf_models/resnet50v2_imagenet.onnx",
      List("Identity:0"),
      options,
      traceContext
    )
  ...
  it should "Compile ONNX float YoloV4-tiny" taggedAs (Slow) in {
    ...
    Compiler.compile(
      name,
      "tf_models/yolov4_tiny_192.onnx",
  ...
  val BiggerArchitecture = Architecture.mkWithDefaults(
    dataType = ArchitectureDataType.FP16BP8,
    arraySize = 16,
    accumulatorDepth = Kibi * 4,
    localDepth = Kibi * 20,
    dram0Depth = Mebi * 16,
    dram1Depth = Mebi * 16,
    stride0Depth = 8,
  ...
  it should "Compile ONNX fixed16bp8 #1" taggedAs (Slow) in {
    val name         = "1_16x16_fixed16bp8_onnx"
    val traceContext = new ExecutiveTraceContext()
    val options = CompilerOptions(
      arch = BiggerArchitecture,
      printProgress = true,
      printSummary = true,
      printLayersSummary = true,
      collectBackendStats = true,
