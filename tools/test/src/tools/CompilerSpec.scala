...
class CompilerSpec extends FlatSpec {
  ...
  it should "Compile TF XOR for 2x2 array with 256 memories and input batch of 1" in {
    ...
    assert(
      getProgramBytes(name) === Array(
        0x00, 0x00, 0x02, 0x42, // DataMove(<-) Local(0) Consts(0) 2(+1)
        0x03, 0x00, 0x00, 0x40, // DataMove(<-) Local(3) Vars(0)
        0x00, 0x02, 0x00, 0x60, // LoadWeights  Local(0) 2(+1)
        0x03, 0x00, 0x00, 0x20, // MatMul       Local(3) Acc(0)
        0x00, 0x00, 0x09, 0x80, // SIMD         R1=0
        0x01, 0x00, 0x7a, 0x83, // SIMD(RW)     O=Max(I,R1) WAcc(1) RAcc(0)
        0x04, 0x01, 0x00, 0x4c, // DataMove(<-) Local(4) Acc(1)
        0x04, 0x01, 0x00, 0x41, // DataMove(->) Local(4) Vars(1)
        0x00, 0x03, 0x02, 0x42, // DataMove(<-) Local(0) Consts(3) 2(+1)
        0x03, 0x01, 0x00, 0x40, // DataMove(<-) Local(3) Vars(1)
        0x00, 0x02, 0x00, 0x60, // LoadWeights  Local(0) 2(+1)
        0x03, 0x00, 0x00, 0x20, // MatMul       Local(3) Acc(0)
        0x04, 0x00, 0x00, 0x4c, // DataMove(<-) Local(4) Acc(0)
        0x04, 0x00, 0x00, 0x41, // DataMove(->) Local(4) Vars(0)
      ).map(_.toByte)
    )
  ...
  it should "Compile TF XOR for 2x2 array with 256 memories and input batch of 4" in {
    ...
    assert(
      getProgramBytes(name) === Array(
        0x00, 0x00, 0x02, 0x42, // DataMove(<-) Local(0) Consts(0) 2(+1)
        0x03, 0x00, 0x03, 0x40, // DataMove(<-) Local(3) Vars(0) 3(+1)
        0x00, 0x02, 0x00, 0x60, // LoadWeights  Local(0) 2(+1)
        0x03, 0x00, 0x03, 0x20, // MatMul       Local(3) Acc(0) 3(+1)
        0x00, 0x00, 0x09, 0x80, // SIMD         R1=0
        0x04, 0x00, 0x7a, 0x83, // SIMD(RW)     O=Max(I,R1) WAcc(4) RAcc(0)
        0x05, 0x01, 0x7a, 0x83, // SIMD(RW)     O=Max(I,R1) WAcc(5) RAcc(1)
        0x06, 0x02, 0x7a, 0x83, // SIMD(RW)     O=Max(I,R1) WAcc(6) RAcc(2)
        0x07, 0x03, 0x7a, 0x83, // SIMD(RW)     O=Max(I,R1) WAcc(7) RAcc(3)
        0x07, 0x04, 0x03, 0x4c, // DataMove(<-) Local(7) Acc(4) 3(+1)
        0x07, 0x04, 0x03, 0x41, // DataMove(->) Local(7) Vars(4) 3(+1)
        0x00, 0x03, 0x02, 0x42, // DataMove(<-) Local(0) Consts(3) 2(+1)
        0x03, 0x04, 0x03, 0x40, // DataMove(<-) Local(3) Vars(4) 3(+1)
        0x00, 0x02, 0x00, 0x60, // LoadWeights  Local(0) 2(+1)
        0x03, 0x00, 0x03, 0x20, // MatMul       Local(3) Acc(0) 3(+1)
        0x07, 0x00, 0x03, 0x4c, // DataMove(<-) Local(7) Acc(0) 3(+1)
        0x07, 0x00, 0x03, 0x41, // DataMove(->) Local(7) Vars(0) 3(+1)
      ).map(_.toByte)
    )
  ...
  it should "Compile TF XOR for 2x2 array with 256 memories and input batch of 16" in {
    ...
    assert(
      getProgramBytes(name) === Array(
        0x00, 0x00, 0x02, 0x42, // DataMove(<-) Local(0) Consts(0) 2(+1)
        0x03, 0x00, 0x0f, 0x40, // DataMove(<-) Local(3) Vars(0) 15(+1)
        0x00, 0x02, 0x00, 0x60, // LoadWeights  Local(0) 2(+1)
        0x03, 0x00, 0x0f, 0x20, // MatMul       Local(3) Acc(0) 15(+1)
        0x00, 0x00, 0x09, 0x80, // SIMD         R1=0
        0x10, 0x00, 0x7a, 0x83, // SIMD(RW)     O=Max(I,R1) WAcc(16) RAcc(0)
        0x11, 0x01, 0x7a, 0x83, // SIMD(RW)     O=Max(I,R1) WAcc(17) RAcc(1)
        0x12, 0x02, 0x7a, 0x83, // SIMD(RW)     O=Max(I,R1) WAcc(18) RAcc(2)
        0x13, 0x03, 0x7a, 0x83, // SIMD(RW)     O=Max(I,R1) WAcc(19) RAcc(3)
        0x14, 0x04, 0x7a, 0x83, // SIMD(RW)     O=Max(I,R1) WAcc(20) RAcc(4)
        0x15, 0x05, 0x7a, 0x83, // SIMD(RW)     O=Max(I,R1) WAcc(21) RAcc(5)
        0x16, 0x06, 0x7a, 0x83, // SIMD(RW)     O=Max(I,R1) WAcc(22) RAcc(6)
        0x17, 0x07, 0x7a, 0x83, // SIMD(RW)     O=Max(I,R1) WAcc(23) RAcc(7)
        0x18, 0x08, 0x7a, 0x83, // SIMD(RW)     O=Max(I,R1) WAcc(24) RAcc(8)
        0x19, 0x09, 0x7a, 0x83, // SIMD(RW)     O=Max(I,R1) WAcc(25) RAcc(9)
        0x1a, 0x0a, 0x7a, 0x83, // SIMD(RW)     O=Max(I,R1) WAcc(26) RAcc(10)
        0x1b, 0x0b, 0x7a, 0x83, // SIMD(RW)     O=Max(I,R1) WAcc(27) RAcc(11)
        0x1c, 0x0c, 0x7a, 0x83, // SIMD(RW)     O=Max(I,R1) WAcc(28) RAcc(12)
        0x1d, 0x0d, 0x7a, 0x83, // SIMD(RW)     O=Max(I,R1) WAcc(29) RAcc(13)
        0x1e, 0x0e, 0x7a, 0x83, // SIMD(RW)     O=Max(I,R1) WAcc(30) RAcc(14)
        0x1f, 0x0f, 0x7a, 0x83, // SIMD(RW)     O=Max(I,R1) WAcc(31) RAcc(15)
        0x13, 0x10, 0x0f, 0x4c, // DataMove(<-) Local(19) Acc(16) 15(+1)
        0x13, 0x10, 0x0f, 0x41, // DataMove(->) Local(19) Vars(16) 15(+1)
        0x00, 0x03, 0x02, 0x42, // DataMove(<-) Local(0) Consts(3) 2(+1)
        0x03, 0x10, 0x0f, 0x40, // DataMove(<-) Local(3) Vars(16) 15(+1)
        0x00, 0x02, 0x00, 0x60, // LoadWeights  Local(0) 2(+1)
        0x03, 0x00, 0x0f, 0x20, // MatMul       Local(3) Acc(0) 15(+1)
        0x13, 0x00, 0x0f, 0x4c, // DataMove(<-) Local(19) Acc(0) 15(+1)
        0x13, 0x00, 0x0f, 0x41, // DataMove(->) Local(19) Vars(0) 15(+1)
      ).map(_.toByte)
    )
  ...
  it should "Compile TF oversized XOR for 4x4 array with 256 memories" in {
    ...
    assert(
      getProgramBytes(name) === Array(
        0x00, 0x00, 0x04, 0x42, // DataMove(<-) Local(0) Consts(0) 4(+1)
        0x05, 0x00, 0x00, 0x40, // DataMove(<-) Local(5) Vars(0)
        0x00, 0x04, 0x00, 0x60, // LoadWeights  Local(0) 4(+1)
        0x05, 0x00, 0x00, 0x20, // MatMul       Local(5) Acc(0)
        0x00, 0x00, 0x09, 0x80, // SIMD         R1=0
        0x01, 0x00, 0x7a, 0x83, // SIMD(RW)     O=Max(I,R1) WAcc(1) RAcc(0)
        0x06, 0x01, 0x00, 0x4c, // DataMove(<-) Local(6) Acc(1)
        0x06, 0x01, 0x00, 0x41, // DataMove(->) Local(6) Vars(1)
        0x00, 0x05, 0x04, 0x42, // DataMove(<-) Local(0) Consts(5) 4(+1)
        0x05, 0x01, 0x00, 0x40, // DataMove(<-) Local(5) Vars(1)
        0x00, 0x04, 0x00, 0x60, // LoadWeights  Local(0) 4(+1)
        0x05, 0x00, 0x00, 0x20, // MatMul       Local(5) Acc(0)
        0x06, 0x00, 0x00, 0x4c, // DataMove(<-) Local(6) Acc(0)
        0x06, 0x00, 0x00, 0x41, // DataMove(->) Local(6) Vars(0)
      ).map(_.toByte)
    )
  ...
  it should "Compile TF XOR for 2x2 array with 64K memories" in {
    ...
    assert(
      getProgramBytes(name) === Array(
        ...
        0x42, // DataMove(<-) Local(0) Consts(0) 2(+1)
        0x03, 0x00, 0x00, 0x00, 0x00, 0x00,
        0x40,                                     // DataMove(<-) Local(3) Vars(0)
        0x00, 0x00, 0x02, 0x00, 0x00, 0x00, 0x60, // LoadWeights  Local(0) 2(+1)
        0x03, 0x00, 0x00, 0x00, 0x00, 0x00,
        0x20,                                     // MatMul       Local(3) Acc(0)
        0x00, 0x00, 0x00, 0x00, 0x09, 0x00, 0x80, // SIMD         R1=0
        0x01, 0x00, 0x00, 0x00, 0x7a, 0x00,
        0x83, // SIMD(RW)     O=Max(I,R1) WAcc(1) RAcc(0)
        0x04, 0x00, 0x01, 0x00, 0x00, 0x00,
        0x4c, // DataMove(<-) Local(4) Acc(1)
        0x04, 0x00, 0x01, 0x00, 0x00, 0x00,
        0x41, // DataMove(->) Local(4) Vars(1)
        0x00, 0x00, 0x03, 0x00, 0x02, 0x00,
        0x42, // DataMove(<-) Local(0) Consts(3) 2(+1)
        0x03, 0x00, 0x01, 0x00, 0x00, 0x00,
        0x40,                                     // DataMove(<-) Local(3) Vars(1)
        0x00, 0x00, 0x02, 0x00, 0x00, 0x00, 0x60, // LoadWeights  Local(0) 2(+1)
        0x03, 0x00, 0x00, 0x00, 0x00, 0x00,
        0x20, // MatMul       Local(3) Acc(0)
        0x04, 0x00, 0x00, 0x00, 0x00, 0x00,
        0x4c, // DataMove(<-) Local(4) Acc(0)
        0x04, 0x00, 0x00, 0x00, 0x00, 0x00,
        0x41, // DataMove(->) Local(4) Vars(0)
      ).map(_.toByte)
    )
  ...
  it should "Compile TF tiled XOR4 for 2x2 array with 128 memories and 2 strides" in {
    ...
    assert(
      getProgramBytes(name) === Array(
        0x00, 0x80, 0x02, 0x42, // DataMove(<-) Local(0) Consts(0)@2^1 2(+1)
        0x03, 0x81, 0x02, 0x42, // DataMove(<-) Local(3) Consts(1)@2^1 2(+1)
        0x06, 0x00, 0x00, 0x40, // DataMove(<-) Local(6) Vars(0)
        0x00, 0x02, 0x00, 0x60, // LoadWeights  Local(0) 2(+1)
        0x06, 0x00, 0x00, 0x20, // MatMul       Local(6) Acc(0)
        0x03, 0x02, 0x00, 0x60, // LoadWeights  Local(3) 2(+1)
        0x06, 0x01, 0x00, 0x20, // MatMul       Local(6) Acc(1)
        0x00, 0x00, 0x09, 0x80, // SIMD         R1=0
        0x02, 0x00, 0x7a, 0x83, // SIMD(RW)     O=Max(I,R1) WAcc(2) RAcc(0)
        0x03, 0x01, 0x7a, 0x83, // SIMD(RW)     O=Max(I,R1) WAcc(3) RAcc(1)
        0x07, 0x02, 0x01, 0x4c, // DataMove(<-) Local(7) Acc(2) 1(+1)
        0x07, 0x01, 0x01, 0x41, // DataMove(->) Local(7) Vars(1) 1(+1)
        0x00, 0x06, 0x04, 0x42, // DataMove(<-) Local(0) Consts(6) 4(+1)
        0x05, 0x01, 0x01, 0x40, // DataMove(<-) Local(5) Vars(1) 1(+1)
        0x01, 0x01, 0x00, 0x60, // LoadWeights  Local(1) 1(+1)
        0x00, 0x00, 0x00, 0x61, // LoadWeights  Zeroes
        0x05, 0x00, 0x00, 0x20, // MatMul       Local(5) Acc(0)
        0x03, 0x01, 0x00, 0x60, // LoadWeights  Local(3) 1(+1)
        0x00, 0x00, 0x00, 0x60, // LoadWeights  Local(0)
        0x06, 0x00, 0x00, 0x21, // MatMul(Acc)  Local(6) Acc(0)
        0x07, 0x00, 0x00, 0x4c, // DataMove(<-) Local(7) Acc(0)
        0x07, 0x00, 0x00, 0x41, // DataMove(->) Local(7) Vars(0)
      ).map(_.toByte)
    )
  ...
  it should "Compile TF tiled XOR4 for 2x2 array with 128 memories, 2 strides and input batch of 4" in {
    ...
    assert(
      getProgramBytes(name) === Array(
        0x00, 0x80, 0x02, 0x42, // DataMove(<-) Local(0) Consts(0)@2^1 2(+1)
        0x03, 0x81, 0x02, 0x42, // DataMove(<-) Local(3) Consts(1)@2^1 2(+1)
        0x06, 0x00, 0x03, 0x40, // DataMove(<-) Local(6) Vars(0) 3(+1)
        0x00, 0x02, 0x00, 0x60, // LoadWeights  Local(0) 2(+1)
        0x06, 0x80, 0x03, 0x20, // MatMul       Local(6) Acc(0)@2^1 3(+1)
        0x03, 0x02, 0x00, 0x60, // LoadWeights  Local(3) 2(+1)
        0x06, 0x81, 0x03, 0x20, // MatMul       Local(6) Acc(1)@2^1 3(+1)
        0x00, 0x00, 0x09, 0x80, // SIMD         R1=0
        0x08, 0x00, 0x7a, 0x83, // SIMD(RW)     O=Max(I,R1) WAcc(8) RAcc(0)
        0x09, 0x01, 0x7a, 0x83, // SIMD(RW)     O=Max(I,R1) WAcc(9) RAcc(1)
        0x0a, 0x02, 0x7a, 0x83, // SIMD(RW)     O=Max(I,R1) WAcc(10) RAcc(2)
        0x0b, 0x03, 0x7a, 0x83, // SIMD(RW)     O=Max(I,R1) WAcc(11) RAcc(3)
        0x0c, 0x04, 0x7a, 0x83, // SIMD(RW)     O=Max(I,R1) WAcc(12) RAcc(4)
        0x0d, 0x05, 0x7a, 0x83, // SIMD(RW)     O=Max(I,R1) WAcc(13) RAcc(5)
        0x0e, 0x06, 0x7a, 0x83, // SIMD(RW)     O=Max(I,R1) WAcc(14) RAcc(6)
        0x0f, 0x07, 0x7a, 0x83, // SIMD(RW)     O=Max(I,R1) WAcc(15) RAcc(7)
        0x0a, 0x08, 0x07, 0x4c, // DataMove(<-) Local(10) Acc(8) 7(+1)
        0x0a, 0x04, 0x07, 0x41, // DataMove(->) Local(10) Vars(4) 7(+1)
        0x00, 0x06, 0x04, 0x42, // DataMove(<-) Local(0) Consts(6) 4(+1)
        0x05, 0x04, 0x07, 0x40, // DataMove(<-) Local(5) Vars(4) 7(+1)
        0x01, 0x01, 0x00, 0x60, // LoadWeights  Local(1) 1(+1)
        0x00, 0x00, 0x00, 0x61, // LoadWeights  Zeroes
        0x85, 0x00, 0x03, 0x20, // MatMul       Local(5)@2^1 Acc(0) 3(+1)
        0x03, 0x01, 0x00, 0x60, // LoadWeights  Local(3) 1(+1)
        0x00, 0x00, 0x00, 0x60, // LoadWeights  Local(0)
        0x86, 0x00, 0x03, 0x21, // MatMul(Acc)  Local(6)@2^1 Acc(0) 3(+1)
        0x0d, 0x00, 0x03, 0x4c, // DataMove(<-) Local(13) Acc(0) 3(+1)
        0x0d, 0x00, 0x03, 0x41, // DataMove(->) Local(13) Vars(0) 3(+1)
      ).map(_.toByte)
    )
  ...
  it should "Compile TF tiled XOR4 for 2x2 array with 128 memories, 2 strides and input batch of 16" in {
    ...
    assert(
      getProgramBytes(name) === Array(
        0x00, 0x80, 0x02, 0x42, // DataMove(<-) Local(0) Consts(0)@2^1 2(+1)
        0x03, 0x81, 0x02, 0x42, // DataMove(<-) Local(3) Consts(1)@2^1 2(+1)
        0x06, 0x00, 0x0f, 0x40, // DataMove(<-) Local(6) Vars(0) 15(+1)
        0x00, 0x02, 0x00, 0x60, // LoadWeights  Local(0) 2(+1)
        0x06, 0x80, 0x0f, 0x20, // MatMul       Local(6) Acc(0)@2^1 15(+1)
        0x03, 0x02, 0x00, 0x60, // LoadWeights  Local(3) 2(+1)
        0x06, 0x81, 0x0f, 0x20, // MatMul       Local(6) Acc(1)@2^1 15(+1)
        0x00, 0x00, 0x09, 0x80, // SIMD         R1=0
        0x20, 0x00, 0x7a, 0x83, // SIMD(RW)     O=Max(I,R1) WAcc(32) RAcc(0)
        0x21, 0x01, 0x7a, 0x83, // SIMD(RW)     O=Max(I,R1) WAcc(33) RAcc(1)
        0x22, 0x02, 0x7a, 0x83, // SIMD(RW)     O=Max(I,R1) WAcc(34) RAcc(2)
        0x23, 0x03, 0x7a, 0x83, // SIMD(RW)     O=Max(I,R1) WAcc(35) RAcc(3)
        0x24, 0x04, 0x7a, 0x83, // SIMD(RW)     O=Max(I,R1) WAcc(36) RAcc(4)
        0x25, 0x05, 0x7a, 0x83, // SIMD(RW)     O=Max(I,R1) WAcc(37) RAcc(5)
        0x26, 0x06, 0x7a, 0x83, // SIMD(RW)     O=Max(I,R1) WAcc(38) RAcc(6)
        0x27, 0x07, 0x7a, 0x83, // SIMD(RW)     O=Max(I,R1) WAcc(39) RAcc(7)
        0x28, 0x08, 0x7a, 0x83, // SIMD(RW)     O=Max(I,R1) WAcc(40) RAcc(8)
        0x29, 0x09, 0x7a, 0x83, // SIMD(RW)     O=Max(I,R1) WAcc(41) RAcc(9)
        0x2a, 0x0a, 0x7a, 0x83, // SIMD(RW)     O=Max(I,R1) WAcc(42) RAcc(10)
        0x2b, 0x0b, 0x7a, 0x83, // SIMD(RW)     O=Max(I,R1) WAcc(43) RAcc(11)
        0x2c, 0x0c, 0x7a, 0x83, // SIMD(RW)     O=Max(I,R1) WAcc(44) RAcc(12)
        0x2d, 0x0d, 0x7a, 0x83, // SIMD(RW)     O=Max(I,R1) WAcc(45) RAcc(13)
        0x2e, 0x0e, 0x7a, 0x83, // SIMD(RW)     O=Max(I,R1) WAcc(46) RAcc(14)
        0x2f, 0x0f, 0x7a, 0x83, // SIMD(RW)     O=Max(I,R1) WAcc(47) RAcc(15)
        0x30, 0x10, 0x7a, 0x83, // SIMD(RW)     O=Max(I,R1) WAcc(48) RAcc(16)
        0x31, 0x11, 0x7a, 0x83, // SIMD(RW)     O=Max(I,R1) WAcc(49) RAcc(17)
        0x32, 0x12, 0x7a, 0x83, // SIMD(RW)     O=Max(I,R1) WAcc(50) RAcc(18)
        0x33, 0x13, 0x7a, 0x83, // SIMD(RW)     O=Max(I,R1) WAcc(51) RAcc(19)
        0x34, 0x14, 0x7a, 0x83, // SIMD(RW)     O=Max(I,R1) WAcc(52) RAcc(20)
        0x35, 0x15, 0x7a, 0x83, // SIMD(RW)     O=Max(I,R1) WAcc(53) RAcc(21)
        0x36, 0x16, 0x7a, 0x83, // SIMD(RW)     O=Max(I,R1) WAcc(54) RAcc(22)
        0x37, 0x17, 0x7a, 0x83, // SIMD(RW)     O=Max(I,R1) WAcc(55) RAcc(23)
        0x38, 0x18, 0x7a, 0x83, // SIMD(RW)     O=Max(I,R1) WAcc(56) RAcc(24)
        0x39, 0x19, 0x7a, 0x83, // SIMD(RW)     O=Max(I,R1) WAcc(57) RAcc(25)
        0x3a, 0x1a, 0x7a, 0x83, // SIMD(RW)     O=Max(I,R1) WAcc(58) RAcc(26)
        0x3b, 0x1b, 0x7a, 0x83, // SIMD(RW)     O=Max(I,R1) WAcc(59) RAcc(27)
        0x3c, 0x1c, 0x7a, 0x83, // SIMD(RW)     O=Max(I,R1) WAcc(60) RAcc(28)
        0x3d, 0x1d, 0x7a, 0x83, // SIMD(RW)     O=Max(I,R1) WAcc(61) RAcc(29)
        0x3e, 0x1e, 0x7a, 0x83, // SIMD(RW)     O=Max(I,R1) WAcc(62) RAcc(30)
        0x3f, 0x1f, 0x7a, 0x83, // SIMD(RW)     O=Max(I,R1) WAcc(63) RAcc(31)
        0x16, 0x20, 0x1f, 0x4c, // DataMove(<-) Local(22) Acc(32) 31(+1)
        0x16, 0x10, 0x1f, 0x41, // DataMove(->) Local(22) Vars(16) 31(+1)
        0x00, 0x06, 0x04, 0x42, // DataMove(<-) Local(0) Consts(6) 4(+1)
        0x05, 0x10, 0x1f, 0x40, // DataMove(<-) Local(5) Vars(16) 31(+1)
        0x01, 0x01, 0x00, 0x60, // LoadWeights  Local(1) 1(+1)
        0x00, 0x00, 0x00, 0x61, // LoadWeights  Zeroes
        0x85, 0x00, 0x0f, 0x20, // MatMul       Local(5)@2^1 Acc(0) 15(+1)
        0x03, 0x01, 0x00, 0x60, // LoadWeights  Local(3) 1(+1)
        0x00, 0x00, 0x00, 0x60, // LoadWeights  Local(0)
        0x86, 0x00, 0x0f, 0x21, // MatMul(Acc)  Local(6)@2^1 Acc(0) 15(+1)
        0x25, 0x00, 0x0f, 0x4c, // DataMove(<-) Local(37) Acc(0) 15(+1)
        0x25, 0x00, 0x0f, 0x41, // DataMove(->) Local(37) Vars(0) 15(+1)
      ).map(_.toByte)
    )
