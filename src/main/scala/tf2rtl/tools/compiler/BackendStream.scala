...
class BackendStream(
  ...
  private def padNoOps(prevOp: UInt, currentOp: UInt): Unit = {
    val key = (prevOp, currentOp)
    if (padding.contains(key)) {
      for (_ <- 0 until padding(key)) {
        emitNoOp()
      }
    }
  }
  ...
  private def beforeEmit(
    ...
    padNoOps(previousOpcode, currentOp)
  ...
  def emitNoOp(
    ...
    emitHeader(Opcode.NoOp.litValue.toByte)
  ...
  def emitMatMul(
    ...
    emitHeader(Opcode.MatMul.litValue.toByte, flags)
  ...
  def emitSIMD(
    ...
    emitHeader(Opcode.SIMD.litValue.toByte, flags)
  ...
  def emitDataMove(
    ...
    emitHeader(Opcode.DataMove.litValue.toByte, flags)
  ...
  def emitLoadWeights(
    ...
    emitHeader(Opcode.LoadWeights.litValue.toByte, flags)
