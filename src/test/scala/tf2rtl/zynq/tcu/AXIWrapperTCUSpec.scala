...
import tf2rtl.data.InstructionReader

import tf2rtl.tools.{Compiler, CompilerOptions}
import tf2rtl.tools.compiler.{InstructionLayout, MemoryAddressHelper}

import tf2rtl.axi
import tf2rtl.tcu.DataFlowControl
import tf2rtl.tcu.instruction.{Opcode, Instruction, DataMoveFlags, DataMoveArgs}
import tf2rtl.mem.MemKind
import tf2rtl.tools.{Util, ResNet}
          ...
          def xor4(batchSize: Int) =
            it(s"should run XOR4 network with $batchSize batch", Slow) {
              test(
                ...
                val modelFileName = "tf_models/xor4.pb"
          ...
          def resnet(batchSize: Int, inputSize: Int) =
            it(
              ...
              test(
                ...
                val modelFileName = "tf_models/resnet20v2_cifar.pb"
                val model         = new FileInputStream(modelFileName)
                val consts        = new ByteArrayOutputStream()
                val program       = new ByteArrayOutputStream()
                ...
                val source = Source.fromFile(
                  s"tf_models/resnet_input_${inputSize}x32x32x${m.arch.arraySize}.csv"
                )
                ...
                for (k <- 0 until inputSize / batchSize) {
                  ...
                  // wait until it's done
                  for (l <- 0 until 8000000 * batchSize) {
